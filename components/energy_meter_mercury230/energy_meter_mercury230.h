...
class Mercury : public PollingComponent  {
    ...
    private:
        ...
        // Сенсоры для тарифов
        Sensor *ValueT1Active {nullptr};
        Sensor *ValueT1Reactive {nullptr};
        Sensor *ValueT2Active {nullptr};
        Sensor *ValueT2Reactive {nullptr};
        Sensor *ValueT3Active {nullptr};
        Sensor *ValueT3Reactive {nullptr};
        Sensor *Temp {nullptr};
        ...
        bool cbValuesT3=false;
        bool cbTemp=false;
        bool cbPhaseEnergy=false;
	...
	public:
        ...
        void set_ValueRC(sensor::Sensor *sens) {this->ValueRC=sens;}
        // Методы для тарифов
        void set_ValueT1Active(sensor::Sensor *sens) {this->ValueT1Active=sens;}
        void set_ValueT1Reactive(sensor::Sensor *sens) {this->ValueT1Reactive=sens;}
        void set_ValueT2Active(sensor::Sensor *sens) {this->ValueT2Active=sens;}
        void set_ValueT2Reactive(sensor::Sensor *sens) {this->ValueT2Reactive=sens;}
        void set_ValueT3Active(sensor::Sensor *sens) {this->ValueT3Active=sens;}
        void set_ValueT3Reactive(sensor::Sensor *sens) {this->ValueT3Reactive=sens;}
        void set_Temp(sensor::Sensor *sens) {this->Temp=sens;}
        // версия
        ...
        void setup() override {
            ...
            if(this->led_active_pin!=nullptr){
               ...
               this->led_active_pin->digital_write(false);
            }
            ...
            cbValuesT1=(this->ValueT1Active!=nullptr || this->ValueT1Reactive!=nullptr);
            cbValuesT2=(this->ValueT2Active!=nullptr || this->ValueT2Reactive!=nullptr);
            cbValuesT3=(this->ValueT3Active!=nullptr || this->ValueT3Reactive!=nullptr);
            cbValuesA=cbValuesT1;
            cbValuesB=cbValuesT2;
            cbValuesC=cbValuesT3;
            cbPhaseEnergy=(this->ValueAA!=nullptr || this->ValueAB!=nullptr || this->ValueAC!=nullptr ||
                          this->ValueRA!=nullptr || this->ValueRB!=nullptr || this->ValueRC!=nullptr);
            cbTemp=(this->Temp!=nullptr);
            debugIn=false;
            debugOut=false;
        ...
        void loop() override {
            ...
            if(this->my_serial!=nullptr){
                ...
                while(this->my_serial->available()){
                    uint8_t rxByte;
                    this->my_serial->read_byte(&rxByte);
                    ESP_LOGV(TAG, "RX byte: 0x%02X", rxByte);
                    getFromMerc(rxByte);
                    rxCount++;
                }
                if(rxCount > 0){
                    ESP_LOGD(TAG, "RX %u bytes received", rxCount);
                }

                uint8_t txLen = availableMerc();
                if(txLen){
                    uint8_t* buff = getBuffForMerc();
                    ESP_LOGD(TAG, "TX %u bytes, DE/RE -> HIGH", txLen);
                    if(this->led_active_pin != nullptr) this->led_active_pin->digital_write(true);
                    this->my_serial->write_array(buff, txLen);
                    this->my_serial->flush();
                    if(this->led_active_pin != nullptr) this->led_active_pin->digital_write(false);
                    ESP_LOGD(TAG, "TX done, DE/RE -> LOW");
                    waiteReply = true;  // arm timeout so counter waits for meter response
                    timeReadByte = millis();  // start timeout from now
                }
            }
        ...
        void update() override {
            ...
            if(oldError != getLastError()){
                oldError = getLastError();
                if(this->error_string!=nullptr){
