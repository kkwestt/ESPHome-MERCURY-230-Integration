...
Mercury230 = Mercury230_ns.class_("Mercury", cg.PollingComponent)
...
initParams = {
    ...
    # реактивная энергия
     ...
     # Энергия по тарифам
     # Тариф 1 - активная энергия
     cv.Optional(CONF_ENERGY+T1+"_active"): sensor.sensor_schema(
        unit_of_measurement=UNIT_KILOWATT_HOURS,
        accuracy_decimals=2,
        device_class=DEVICE_CLASS_ENERGY,
        state_class=STATE_CLASS_TOTAL_INCREASING,
     ),
     # Тариф 1 - реактивная энергия
     cv.Optional(CONF_ENERGY+T1+"_reactive"): sensor.sensor_schema(
        unit_of_measurement=UNIT_KILOWATT_HOURS,
        accuracy_decimals=2,
        device_class=DEVICE_CLASS_ENERGY,
        state_class=STATE_CLASS_TOTAL_INCREASING,
     ),
     # Тариф 2 - активная энергия
     cv.Optional(CONF_ENERGY+T2+"_active"): sensor.sensor_schema(
        unit_of_measurement=UNIT_KILOWATT_HOURS,
        accuracy_decimals=2,
        device_class=DEVICE_CLASS_ENERGY,
        state_class=STATE_CLASS_TOTAL_INCREASING,
     ),
     # Тариф 2 - реактивная энергия
     cv.Optional(CONF_ENERGY+T2+"_reactive"): sensor.sensor_schema(
        unit_of_measurement=UNIT_KILOWATT_HOURS,
        accuracy_decimals=2,
        device_class=DEVICE_CLASS_ENERGY,
        state_class=STATE_CLASS_TOTAL_INCREASING,
     ),
     # Тариф 3 - активная энергия
     cv.Optional(CONF_ENERGY+T3+"_active"): sensor.sensor_schema(
        unit_of_measurement=UNIT_KILOWATT_HOURS,
        accuracy_decimals=2,
        device_class=DEVICE_CLASS_ENERGY,
        state_class=STATE_CLASS_TOTAL_INCREASING,
     ),
     # Тариф 3 - реактивная энергия
     cv.Optional(CONF_ENERGY+T3+"_reactive"): sensor.sensor_schema(
        unit_of_measurement=UNIT_KILOWATT_HOURS,
        accuracy_decimals=2,
        device_class=DEVICE_CLASS_ENERGY,
        state_class=STATE_CLASS_TOTAL_INCREASING,
     ...
     # адрес счетчика
     cv.Optional(CONF_USE_ADDRESS): cv.int_range(min=0, max=240),
     # update interval
...
CONFIG_SCHEMA = cv.All(cv.Schema(initParams).extend(uart.UART_DEVICE_SCHEMA).extend(cv.COMPONENT_SCHEMA), output_info)
...
async def to_code(config):
    ...
    # Энергия по тарифам
    # Тариф 1
    if (CONF_ENERGY+T1+"_active") in config:
        sens = await sensor.new_sensor(config[CONF_ENERGY+T1+"_active"])
        cg.add(var.set_ValueT1Active(sens))
    if (CONF_ENERGY+T1+"_reactive") in config:
        sens = await sensor.new_sensor(config[CONF_ENERGY+T1+"_reactive"])
        cg.add(var.set_ValueT1Reactive(sens))
    # Тариф 2
    if (CONF_ENERGY+T2+"_active") in config:
        sens = await sensor.new_sensor(config[CONF_ENERGY+T2+"_active"])
        cg.add(var.set_ValueT2Active(sens))
    if (CONF_ENERGY+T2+"_reactive") in config:
        sens = await sensor.new_sensor(config[CONF_ENERGY+T2+"_reactive"])
        cg.add(var.set_ValueT2Reactive(sens))
    # Тариф 3
    if (CONF_ENERGY+T3+"_active") in config:
        sens = await sensor.new_sensor(config[CONF_ENERGY+T3+"_active"])
        cg.add(var.set_ValueT3Active(sens))
    if (CONF_ENERGY+T3+"_reactive") in config:
        sens = await sensor.new_sensor(config[CONF_ENERGY+T3+"_reactive"])
        cg.add(var.set_ValueT3Reactive(sens))
